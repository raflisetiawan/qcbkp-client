<template>
  <div>
    <q-dialog v-model="$state.showAddInformationDialog" maximized transition-show="slide-up"
...
const { $state, addInformation } = useInformationStore()
...
const onSubmit = async (): Promise<void> => {
  if (!v$.value.$invalid) {
    try {
      loading.value = true;
      await addInformation(addInformationForm)
    } catch (error) {
      ...
      loading.value = false;
      $state.showAddInformationDialog = false
    }
  } else {
    v$.value.$touch();
  }
