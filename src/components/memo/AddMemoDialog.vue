<template>
  <div>
    <q-dialog v-model="$state.showAddMemoDialog" maximized transition-show="slide-up" transition-hide="slide-down">
      <q-card>
        ...
        <q-card-section>
          <div class="text-h6">Buat Memo</div>
        </q-card-section>
...
const { $state, addMemo } = useMemoStore()
...
const onSubmit = async (): Promise<void> => {
  if (!v$.value.$invalid) {
    try {
      loading.value = true;
      await addMemo({ ...addMemoForm, issue_id: $state.issue_id })
    } catch (error) {
